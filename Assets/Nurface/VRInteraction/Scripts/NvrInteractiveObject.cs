...
public class NvrInteractiveObject : MonoBehaviour {
    ...
    void Start(){
        ...
        timerGaze = GameObject.FindWithTag("GazeTimerText").GetComponent<Text>();
        timerGaze.transform.localScale = Vector3.zero;
        timerGaze.text = gazeClickTime.ToString("0"); 
    }
    ...
    private void Update() {
        ...
        if (interactionEnabled == false || clickWithGaze == false) {
            ...
            timerGaze.transform.localScale = Vector3.one;
            timerGaze.text = timer.ToString("0");
